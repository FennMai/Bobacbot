    ...
    def inference_loop(self, policy):
        ...
        while True:
            ...
            if self.act_queue.qsize() < LATENCY_STEPS and len(obs_history) == self.n_obs_steps:
                print(f"[{time.time():.3f}] Inference loop: Starting policy step.") # DEBUG
                obs_sequence = list(obs_history)
                act_sequence = policy.step(obs_sequence)
                print(f"[{time.time():.3f}] Inference loop: Policy step returned {len(act_sequence)} actions.") # DEBUG
                ...
                print(f"[{time.time():.3f}] Inference loop: Finished queuing actions.") # DEBUG
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--ckpt-path', default='data/outputs/2025.06.15/12.29.57_train_diffusion_unet_hybrid_sim_pick_place/checkpoints/epoch=0600-train_loss=0.001.ckpt')
    parser.add_argument('--ckpt-path', default='data/outputs/2025.07.14/00.30.40_train_diffusion_unet_hybrid_sim_pick_place_v2_July/checkpoints/latest.ckpt')  
